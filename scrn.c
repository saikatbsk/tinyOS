...
/*
 * Updates the hardware cursor: the little blinking line on the screen
 * under the last character pressed!
 */
void move_csr(void) {
    unsigned temp;

    /*The equation for finding the index in a linear chunk of memory can
     * be represented by: Index = [(y * width) + x]
     */
    temp = csr_y * COLS + csr_x;

    /* This sends a command to indicies 14 and 15 in the CRT Control Register
...
void cls() {
    ...
    /* Again, we need the 'short' that will be used to represent a
     * space with color
     */
    blank = 0x20 | (attrib << 8);
...
void putch(unsigned char c) {
     ...
     * as if a 'CR' was also there, so we bring the cursor to the margin
     * and we increment the 'y' value
     */
    else if(c == '\n') {
        csr_x = 0;
        csr_y++;
     ...
     * of memory can be represented by: Index = [(y * width) + x]
     */
    else if(c >= ' ') {
...
void settextcolor(unsigned char forecolor, unsigned char backcolor) {
    /*
     * Top 4 bytes are the background, bottom 4 bytes are the
     * foreground color
     */
